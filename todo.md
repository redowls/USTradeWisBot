...
3. **Five files uncommitted for seven weeks** (`bot/analytics.py`, `bot/exit_sim.py`, `bot/replay.py`, `scripts/replay.py`, `tests/test_replay.py`). `bot/exit_sim.py` feeds the 09-08 IMP-040 verdict and is not in version control. The weekly routine may stage only memory files, so it cannot fix this.
   - ⚠️ **MEASURED CONSEQUENCE (daily review 2026-09-11, reviewing 09-10) — `origin/main` does not pass its own test suite and has not since `b03f5ce`.** Checked out clean, **HEAD scores 20 failed / 596 passed**; every failure is `tests/test_exit_sim.py`, which is **committed** and expects a `high_price` parameter that exists only in the **uncommitted** `bot/exit_sim.py`. `b03f5ce "housekeeping: commit pre-existing IMP-031 ratchet parity test"` committed the test without its source change.
   - **Blast radius is simulation only** — `bot/exit_sim.py` is imported by no live-path module (`bot/engine.py`, `bot/exits.py`, `bot/strategy.py`, `main.py` are all clear), so the running service is unaffected and there is no trading risk. ★ **But every recent run's "full suite passes" was measured against a working tree that is not what is in git**, which is why this went unnoticed for seven weeks.
   - **A daily run cannot fix it either** — the standing rule forbids staging files it did not modify. **Needs a human `git add` of the five files (or a decision to discard them).**

### Update 2026-09-15 (daily review of 2026-09-14) — item 2 is now the ONLY open strategy question

⚠️ **IMP-055 refuted the last remaining non-entry hypothesis, so the retire-or-rebuild call (item 2 above) no longer has any pending technical work in front of it.** The week-ending-09-11 weekly designated one live-path change — the `ATR_STOP_MULT`/`MIN_STOP_PCT` refit ("make 1R scale with the name's realized daily range instead of a 1.5% constant") — and required it be re-derived against IMP-054's WIN ceiling before shipping. It was, over the full post-gate book (n=122, 35 sessions, SIP bars, live ratchet, dollar risk held constant), and **it fails**:

- **Tightening 1R from 1.57% to 0.79% raises the number of trades from which a +1R win is arithmetically available from 20 to 49 (+145%) and converts 4 → 3.** Conversion rate collapses 20.0% → 6.1%.
- **Payoff falls monotonically at every step in both policy families** (uniform-scale 0.61 → 0.40; ADR-cap 0.54 → 0.32). The live 1.57% stop has the best payoff in the grid.
- True win rate is inert across the whole grid (2.5%–4.9%).
- ⚠️ The `stop_distance% ÷ ADR20%` entry gate pre-registered 2026-09-04 **is** the ADR-cap family above and should be treated as **formally withdrawn**, not merely unexamined.

**Every layer downstream of the entry has now been tested and exonerated on its own evidence:** the exit (IMP-054 — 83.5% of the book unwinnable from the fill, 40% conversion where the move existed), the VWAP gate (four consecutive audits; 2026-09-14's was on a strongly trending tape and found **85.5% of 83 scoreable refusals unwinnable**), the break-even/trail ratchet (trailing-10: `break-even` net +$15.57 and `trailed-scratch` net +$86.57, with the entire −$144.26 loss in four trades that never reached +0.25R), and now the stop geometry. **Sixteen entry discriminators have been refuted. There is no remaining knob outside the entry signal.**

★ **The binary the weekly set — "week ten must change the live path or retire the bot" — is answered: the designated live-path change does not work.** Item 2 is a capital-allocation decision and it is the fifth consecutive week of asking. **No risk limit is being proposed for change; nothing here loosens anything.**
