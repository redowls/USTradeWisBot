...
- ⚠️ **Process defect, third occurrence: the routine keeps dying between shipping code and recording it.** IMP-031 (written, live, never committed), IMP-034 (written, live, never committed *or* deployed), and now **IMP-036 (written, validated, committed and pushed — but with no improvement-log entry and no daily review)**. The failure mode has now hit in all three possible orders. **The running process is unaffected in this instance** — IMP-036 touched only `bot/discriminator.py`, `scripts/entry_discriminator.py` and `tests/test_discriminator.py`, and nothing in the live path imports any of them (verified by grep), so no restart was owed and the bot has been bit-identical throughout. **This is a record-keeping failure, not a deployment failure — but it is the same broken step.** Escalated in `todo.md` since 08-14 and still unaddressed.

### Lessons & improvement candidates
1. **★ NO NEW CODE CHANGE THIS RUN — and this is a decision, not a default.** Four independent reasons, any one sufficient: **(a) 08-18's improvement already exists and shipped** — IMP-036, commit `d96d6ff`, pushed 20:49:49 UTC that evening. One improvement per run; 08-18's is spent, and the missing artefact is the *record*, which this entry supplies. **(b) Every candidate 08-18's data points at is already refuted or explicitly deferred** — the MFE-giveback shape (42-cell grid, refuted by IMP-033), the +0.5R trigger (swept by IMP-032, all noise), the range-vs-1R mismatch (NOT SUPPORTED by IMP-036 today), holding stopped trades to the flatten (risk-widening, needs human approval). **(c) Three IMPs have open live reads** — IMP-034 (first decisive case today), IMP-035 (second null-pass session), IMP-036 (shipped last night). Stacking a fourth destroys attribution on all three. **(d) The 08-19 session opens in ~3h40m into FOMC minutes at 14:00 ET.** Shipping an untested strategy change into that, hours before the pre-market run, would be reckless. **"Reviewed, no change warranted" is the honest outcome.**
